...
**a) Isi partner_id & partner_key toko itu:**
```bash
curl -X POST -H "x-admin-secret: ISI_ADMIN_SECRET" -H "Content-Type: application/json" \
  -d '{"shop":"diansari_plastik","partner_id":"ANGKA_PARTNER_ID_TOKO_INI","partner_key":"PARTNER_KEY_TOKO_INI"}' \
  "https://NAMA-PROJECT.vercel.app/api/admin/set-partner"
```
Ambil `partner_id`/`partner_key` dari App yang bersangkutan di open.shopee.com > My Apps.
