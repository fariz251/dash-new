...
## Soal batas waktu Vercel (penting)

Paket gratis (Hobby) Vercel **defaultnya cuma kasih 10 detik per request** untuk Serverless
Function — bukan 60 detik seperti nilai `maxDuration` di `vercel.json` (itu cuma batas atas
konfigurasi yg diizinkan, bukan jaminan). Kalau sering kena `FUNCTION_INVOCATION_TIMEOUT`:

1. **Aktifkan Fluid Compute** (gratis): Vercel Dashboard → Project → Settings → Functions →
   toggle **Fluid Compute** → redeploy. Ini menaikkan batas jadi 300 detik.
2. Tanpa Fluid Compute, backfill sudah didesain memproses **1 hari per klik** supaya tetap
   aman di bawah 10 detik untuk toko dengan volume order wajar.

## Total Dicairkan / Estimasi

- **Sebelum disync**: `total_dicairkan` = **estimasi** (disamakan dengan omset hari itu), ditandai `EST` di dashboard. Ini jauh lebih masuk akal daripada 0.
- **Setelah disync** (order berstatus COMPLETED): diganti nilai **asli** dari `get_escrow_detail` Shopee, tanda `EST` hilang utk tanggal yg semua order-nya sudah settle.

Jalankan sync escrow lewat browser (aman diulang, idempotent):
```
https://NAMA-PROJECT.vercel.app/api/cron/sync-escrow?admin_secret=ISI_ADMIN_SECRET&limit=15
```
Lalu jadwalkan juga di cron-job.org (tiap ±2 jam) pakai `CRON_SECRET`:
```
https://NAMA-PROJECT.vercel.app/api/cron/sync-escrow?token=ISI_CRON_SECRET
```

**Catatan jujur:** nama field response `get_escrow_detail` (`escrow_amount`, dll) saya
implementasikan best-effort berdasarkan pola umum Shopee v2 — **belum 100% saya verifikasi**
ke response asli (saya tidak bisa memanggil API Shopee langsung dari sini). Kalau setelah
dijalankan angkanya kelihatan aneh/nol terus, buka respons JSON dari `sync-escrow` di atas,
cari field `_raw_response_sample`, dan **tempel ke saya** — saya sesuaikan
`extractEscrowAmount()` di `lib/escrowSync.js` supaya pas dgn field yang benar.

## Backfill otomatis lewat dashboard

Selain lewat browser manual, sekarang ada tombol **▶ Mulai Backfill Bulan Ini** di dashboard
(butuh token admin) yang otomatis lanjut sendiri sampai selesai, tidak perlu klik berkali-kali.

## Tarik data historis sekali (backfill, manual/API)

Cron 15-menit hanya menangkap order **baru sejak dijalankan** — order yang sudah lewat
sebelum Anda setup tidak otomatis muncul. Untuk itu, jalankan backfill lewat browser:

```
https://NAMA-PROJECT.vercel.app/api/admin/backfill?shop=mitra_diansari&admin_secret=ISI_ADMIN_SECRET
```
Defaultnya mulai dari **awal bulan berjalan**. Mau rentang lain? Tambah `&days_back=30`
(maks 90 hari ke belakang).

Supaya tidak kena timeout Vercel, tiap panggilan cuma memproses **4 hari sekaligus**. Kalau
responsnya `"done": false`, ada field `lanjutkan_dengan_url` — buka/klik link itu untuk
lanjut ke potongan berikutnya, ulangi sampai muncul `"done": true`. Aman diulang berkali-kali
(idempotent, tidak dobel hitung).
