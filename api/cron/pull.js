...
module.exports = async (req, res) => {
  // Proteksi: hanya scheduler yg tahu CRON_SECRET yg boleh trigger
  ...
  const results = [];
  const errors = [];
  for (const shop of getShopConfigs()) {
    if (!shop.shopId || !shop.accessToken) {
      errors.push({ shop: shop.code, error: 'Kredensial belum diisi di environment variables' });
      continue;
    }
    try {
